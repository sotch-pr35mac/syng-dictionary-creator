...
use dictionary_utils::{calculate_hash, MeasureWord, WordEntry};
use hsk::Hsk;
...
fn get_tone_marks(pinyin: &str) -> Vec<u8> {
    ...
    for elem in re.captures_iter(pinyin) {
        ...
        if tone == (5_u8) || tone == (4_u8) || tone == (3_u8) || tone == (2_u8) || tone == (1_u8) {
...
fn is_measure_word(line: &str) -> bool {
    line.to_string().chars().skip(0).take(3).collect::<String>() == *"CL:"
}

fn prune_measure_words(english: Vec<String>) -> Vec<String> {
    return english
        .iter()
        .filter(|&x| !is_measure_word(x))
        .map(|x| x.to_string())
        .collect::<Vec<String>>();
}
...
fn process_cedict_entry(raw_line: &str) -> CEDictEntry {
    ...
    let pinyin = &line[pinyin_start..pinyin_end]
        .split('[')
        .collect::<String>();
