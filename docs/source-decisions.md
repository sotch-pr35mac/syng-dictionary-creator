...
The source revisions, checksums, license evidence, parser versions, and attribution strings are maintained in `sources.lock.json`. Representative regression cases live beside the Rust adapters and identity implementation as unit tests, so they cannot drift away from executable behavior.
