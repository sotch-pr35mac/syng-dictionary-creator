...
fn build_searchable_english(english: &Vec<String>) -> Vec<String> {
    ...
    for term in english {
        ...
        if formatted.chars().take(3).collect::<String>() == *"to " {
...
pub fn build_dictionary(word_list: Vec<WordEntry>) -> SyngDictionary {
    ...
    for (id, mut entry) in (0_u32..).zip(word_list.into_iter()) {
        dictionary
            .traditional
        ...
        let searchable_pinyin: Vec<String> = build_searchable_pinyin(
            (entry.pinyin_marks).to_string(),
            (entry.pinyin_numbers).to_string(),
        );
